...
export function usePostCreate(options?: MutationOptions<Post>){
    ...
    const {mutate, isLoading, isError} = useMutation<Post, unknown, {text: string; imageCover: ImageForUpload}>({
    ...
    function createPost({
        ...
        const imageCover = multimediaService.prepareForUpload(imageUri);
